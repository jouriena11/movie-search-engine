...
function loadLocalStore() {
    ...
    if (localStoreBroadSearch250 !== null) {
        ...
        fetchFilterData = localStoreBroadSearch250.result;
        renderPagination();
        renderSearchResults(broadSearch250.result, 1);
    } else {
        searchMoviePlot(['']);
    }
}
...
function plotSearchFilter(normalSearchQuery) {
    for (let i = 0; i < normalSearchQuery.length; i++) {
        for (let j = 0; j < broadSearch250.result.length; j++) {
            ...
            if(moviePlot.search(normalSearchQuery[i]) !== -1 ) { // -1 means no match is found
                fetchFilterData.push(broadSearch250.result[j])
            }
        }
    }
    renderPagination()
    renderSearchResults(fetchFilterData, 1);
}
        ...
        fetch(`https://imdb-api.com/API/AdvancedSearch/k_lyhir636?title_type=feature&user_rating=7.0,10&num_votes=50000,&languages=en&count=250&sort=user_rating,desc`)
            ...
            .then(function(data) {
                if(!data){
                    ...
                    plotSearchFilter(normalSearchQuery);
                }
            })
    }
...
//     let previousPaginationBtn = document.getElementById(`pagination-btn-${currentPage}`)
//     previousPaginationBtn.classList.remove('active');
//     let activePaginationBtn = undefined;

//     if(type == 'prev') {
//         activePaginationBtn = document.getElementById(`pagination-btn-${currentPage - 1}`);
//         currentPage = currentPage - 1;
//     } else if (type == 'next') {
//         activePaginationBtn = document.getElementById(`pagination-btn-${currentPage + 1}`);
//         currentPage = currentPage + 1;
//     } else {
//         activePaginationBtn = document.getElementById(`pagination-btn-${page}`); // <li>
//         currentPage = page;
//     }
//     renderPagination();
//     activePaginationBtn.classList.add('active');
//     renderSearchResults(fetchFilterData, currentPage);
//     // active = 3
// }
...
function renderPagination() {
    const paginationElem = document.getElementById('movie-pagination');
    const pageCount = Math.ceil(fetchFilterData.length/12);

    let paginationHTML = `<li class="page-item ${currentPage == 1 ? 'disabled': ''}"><a class="page-link" onclick="goToPage('prev', -1)" href="#section-2-heading">Previous</a></li>`;
    for (let i = 0; i < pageCount; i++) { // 21
        if(i+1 == 1 || i+1 == pageCount) {
            paginationHTML += `<li id="pagination-btn-${i+1}" class="page-item ${i+1 == currentPage ? 'active': ''}"><a class="page-link" onclick="goToPage('goto', ${i+1})" href="#section-2-heading">${i+1}</a></li>`;
        }
        else if(i+1 == currentPage - 4 || i+1 == currentPage + 4) {
            paginationHTML += `<li class="page-item disabled"><a class="page-link" href="#pagination-dots">...</a></li>`;
        }
    ...
    paginationHTML += `<li class="page-item ${currentPage == pageCount ? 'disabled': ''}"><a class="page-link" onclick="goToPage('next', -1)" href="#section-2-heading">Next</a></li>`;
    
    paginationElem.innerHTML = paginationHTML;

}

function renderSearchResults(filterPlotList, page) { // filterPlotList = 80 , page = 1
    const loadingSpinner = document.getElementById('loading-spinner');
    filterPlotList = filterPlotList.slice(page*12-12 , page*12); 
    searchResultContainer.innerHTML = '';
    ...
    .then(function(data) {
        if(!data){
            ...
            if(runtime.length !== 0){
                ...
                fetchFilterData = advancedFilterList;
                renderPagination();
                renderSearchResults(advancedFilterList, 1);
            }
        }
    })
}
